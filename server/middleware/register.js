...
User.findOne({where: { email: input.username }, attributes: ['id','email', 'email_confirmed','password']})
  .then(user => {
    if (user==null) {
      var pw = passwordCrypto.hashPassword(input.password, function (err, isHashed){
        if (!err && isHashed){
